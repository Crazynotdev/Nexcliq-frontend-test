...
export function LandingScreen({ onLogin, onRegister }) {
  ...
  const theme = {
    bg: isDark ? '#04080B' : '#FCFDFD',
    surface: isDark ? '#0A1116' : '#F2F4F5',
    text: isDark ? '#ECEEF0' : '#0B0E11',
    text2: isDark ? '#6B7A89' : '#5A6874',
    border: isDark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.04)',
    glass: isDark ? 'rgba(255,255,255,0.03)' : 'rgba(255,255,255,0.7)',
    green: '#0B6B5C',
    greenGlow: 'rgba(11,107,92,0.3)',
    sand: '#C4B49A',
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              {[['first_name', 'Prénom'], ['last_name', 'Nom']].map(([k, l]) => (
                <div key={k}>
                  ...
                  <input placeholder={k === 'first_name' ? 'cletus' : 'junior'} value={form[k]} onChange={e => setForm(f => ({...f, [k]: e.target.value}))} style={{ width: '100%', padding: '13px', borderRadius: 12, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', fontSize: 14, outline: 'none', fontFamily: 'inherit' }} />
                </div>
              ))}
            </div>
